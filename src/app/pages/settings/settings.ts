...
export class Settings implements OnDestroy {
  ...
  readonly comments = signal<UserComment[]>([]);
  ...
  readonly currentUsername = this.profileStore.username;
  ...
  private readonly authRedirectEffect = effect(() => {
    const resolved = this.auth.sessionResolved();
  ...
  private async loadProfile(): Promise<void> {
    try {
      ...
      if (!profile) {
        this.errorMessage.set('Unable to find your account profile.');
      ...
      this.errorMessage.set('Unable to load your account settings.');
    }
  }
  ...
  private async loadComments(): Promise<void> {
    ...
    try {
      ...
      this.commentsNextCursor.set(response.nextCursor);
    } catch (error) {
      console.error(error);

      this.commentsError.set('Unable to load your comments.');
    } finally {
  ...
  async loadMoreComments(): Promise<void> {
    ...
    if (!cursor || this.commentsLoadingMore()) {
      return;
    }
    ...
    try {
      const response = await this.commentsApi.getCurrentUserComments(cursor);
  ...
  async updateUsername(): Promise<void> {
    ...
    if (trimmedUsername === this.currentUsername()) {
      this.successMessage.set('Your username is already set to that.');
    ...
    try {
      const profile = await this.userApi.updateUsername(trimmedUsername);

      this.profileStore.updateUsername(profile.username);

      this.successMessage.set(`Username changed to ${profile.username}.`);
    } catch (error: any) {
      console.error(error);
      ...
      if (error.status === 409) {
        ...
        this.errorMessage.set('Unable to update username.');
      }
    } finally {
