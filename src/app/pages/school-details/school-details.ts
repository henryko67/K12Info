...
export class SchoolDetails implements OnDestroy {
  ...
  readonly expandedDetails =
    signal<SchoolDetailsResponse | null>(null);
  ...
  private openCommentStream(
    sector: 'public' | 'private',
    schoolId: string
  ): void {
    this.commentStream?.close();

    this.commentStream = new EventSource(
      `/api/schools/${sector}/${schoolId}/comments/stream`
    );

    this.commentStream.addEventListener(
      'comment-created',
      async event => {
        ...
        try {
          const response =
            await this.commentsApi.getSchoolComments(
              sector,
              schoolId
            );
    ...
    this.commentStream.addEventListener(
      ...
      event => {
        ...
        void this.handleCommentDeleted(
          data.comment_id
        );
      }
    );
  ...
  private async loadSchool(): Promise<void> {
    const sector =
      this.route.snapshot.paramMap.get('sector');
    ...
    try {
      let school =
        this.explorerStore.selectedSchool();
      ...
      if (
        ...
        school = await firstValueFrom(
          this.schoolsApi.getSchoolById(
            sector,
            id
          )
        );
      ...
      this.openCommentStream(
        school.sector,
        school._id
      );

      if (school.sector === 'public') {
        await this.loadExpandedDetails(
          school.ids.ncessch
        );
      }
    } catch (error) {
  ...
  private async loadExpandedDetails(
    ncessch: string
  ): Promise<void> {
    const cachedDetails =
    ...
    const details = await firstValueFrom(
      this.schoolDetailsApi.getDetails(
        ncessch
      )
    );
    ...
    this.explorerStore.setSchoolDetails(
      ncessch,
      details
    );
  }
  ...
  async postComment(): Promise<void> {
    ...
    try {
      await this.commentsApi.createComment(
        school.sector,
        school._id,
        text
      );
